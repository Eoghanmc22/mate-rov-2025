...
impl Plugin for LearnReferenceFramePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<LearningState>().add_systems(
            Update,
            (
...
fn latency_estimator(mut state: ResMut<LearningState>) {
    if state.waterlinked_state.state == VelocityState::Moving
        ...
        && state.waterlinked_state.last_time - state.robot_state.start_time
    ...
    {
        let latency_secs =
            (state.waterlinked_state.start_time - state.robot_state.start_time).as_secs_f32();
    ...
    if let Some((last_pose, last_pose_time)) = *last_waterlinked_pose {
        ...
        state
            ...
            .retain(|(_velo, _thrust, time)| {
                *time > current_time - latency_estimate - TIME_EPSILON
            });
...
fn learn_reference_frame(
    ...
    let velo_idx = state
        .robot_velocity_buffer
        .partition_point(|(_, _, time)| *time < current_time - state.position_latency_estimate);
    ...
    if let (Some((velo, _, _)), Some((_, thrust, _))) = (velo, thrust) {
        ...
        if USE_REFERENCE_FRAME_LEARNING {
            if Quat::angle_between(reference_angle.0, observed_rotation) < LEARN_MAX_DIFFERENCE {
