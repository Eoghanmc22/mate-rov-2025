...
use kfilter::{
    ...
    system::{LinearNoInputSystem, LinearSystem, System},
    Kalman, KalmanFilter,
};
...
pub type Dynamics = LinearNoInputSystem<f32, { state_constants::NUM_STATES }>;
...
pub fn make_filter(config: &KalmanConfig, delta_t: f32, initial_position: Option<Vec3A>) -> Filter {
    ...
    init_cov_diag[state_constants::ACC_X] = 5.0;
    init_cov_diag[state_constants::ACC_Y] = 5.0;
    init_cov_diag[state_constants::ACC_Z] = 5.0;
...
pub fn make_system(
    ...
    process_covariance[(state_constants::POS_X, state_constants::POS_X)] = config.pos_process_noise;
    process_covariance[(state_constants::POS_Y, state_constants::POS_Y)] = config.pos_process_noise;
    process_covariance[(state_constants::POS_Z, state_constants::POS_Z)] = config.pos_process_noise;
    ...
    LinearNoInputSystem::new(transition, process_covariance, state)
}
...
mod tests {
    ...
    use bevy::math::Vec3A;
    use kfilter::{Kalman, KalmanFilter, KalmanPredict, KalmanPredictInput, KalmanUpdate};
    use nalgebra::{OMatrix, OVector, SVector};
    use rand::random;
    use rand_distr::{Distribution, Normal};

    use crate::kalman_filter::kfilter_model::{
