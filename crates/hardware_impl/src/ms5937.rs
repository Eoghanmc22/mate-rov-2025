...
use generic::{
    Hardware,
    device::{depth::DepthSensor, temperature::TemperatureSensor},
...
pub struct Ms5837 {
    ...
    last_frame: Option<Ms5836Frame>,
    initialized: bool,
}

#[derive(Debug, Clone)]
pub struct Ms5836Frame {
    pub depth: Meters,
    pub altitude: Meters,
    pub pressure: Mbar,
    pub temp: Celsius,
...
impl Ms5837 {
    ...
    pub fn read_frame(&mut self) -> anyhow::Result<Ms5836Frame> {
        let raw = self.read_raw().context("Read raw frame")?;

        let (pressure, temp) = calculate_pressure_and_temperature(raw, &self.calibration);
...
impl Hardware for Ms5837 {
    ...
    fn poll(&mut self) -> anyhow::Result<()> {
        self.read_frame().context("Read frame Ms5937")?;
...
impl DepthSensor for Ms5837 {
    fn read_depth(&self) -> anyhow::Result<Meters> {
        if let Some(last_frame) = &self.last_frame {
            return Ok(last_frame.depth.clone());
        }
    ...
    fn read_altitude(&self) -> anyhow::Result<Meters> {
        if let Some(last_frame) = &self.last_frame {
            return Ok(last_frame.altitude.clone());
        }
    ...
    fn read_pressure(&self) -> anyhow::Result<Mbar> {
        if let Some(last_frame) = &self.last_frame {
            return Ok(last_frame.pressure.clone());
        }
...
impl TemperatureSensor for Ms5837 {
