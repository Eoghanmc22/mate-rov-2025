...
use crate::{
    InstanceName,
    ecs_sync::protocol::Protocol,
    ecs_sync::{
        EntityMap, ForignOwned, NetId, NetTypeId, SerializationSettings, SerializedChange,
        SerializedChangeInEvent, SerializedChangeOutEvent, apply_changes::ChangeApplicationSet,
        detect_changes::ChangeDetectionSet,
    },
    serialization,
};
...
use simple_networking::{Event as NetEvent, Messenger, Networking, Token as NetToken};
    ...
    for event in browse.0.try_iter() {
        match event {
            ServiceEvent::ServiceResolved(info) => {
                ...
                peers
                    .0
                    .insert(info.get_fullname().to_owned(), DiscoveredPeer {
                        info,
                        addresses,
                    });
            }
...
struct Deltas {
    entities: HashMap<NetId, HashMap<NetTypeId, serialization::BackingType>>,
}
